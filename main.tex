...
\begin{enumerate}
    ...
    \item \textit{Pitch}, calculada em Hz a partir da frequência fundamental (\textit{f0}) média, calculada usando o método \textsc{swipe} da biblioteca \textit{pysptk}, aplicado em janelas de 512 amostras com limite de frequências entre 75 e 600 Hz.
...
\begin{enumerate}
    \item Se atualmente as \textit{features} estão organizadas em torno da fala, futuramente faria sentido reorientá-las em direção à audição. \citet{livroDoPlinio} discute certas diferenças desses dois polos e seria importante explorar compensações que levem em conta efeitos psicoacústicos conhecidos -- e.g. a resposta de frequências do ouvido humano é não-linear, o que pode ser melhor modelado usando-se semitons ao invés de Hz; variações de \textit{f0} causam a percepção de maior duração; a sensação de \textit{pitches} agudos é amplificada por maiores volumes; etc.
...
Para o primeiro experimento, aplicamos o seguinte mapeamento de parâmetros prosódicos para eixos tipográficos:

\begin{itemize}
    \item Amplitude para \textit{Weight} (peso);
    \item Frequência fundamental para \textit{Slant} (inclinação);
    \item Duração para \textit{Width} (largura horizontal);
\end{itemize}

O propósito da primeira avaliação foi investigar que influências nosso modelo prosódico-tipográfico teria na interpretação de um texto. Para isolar os efeitos da forma tipográfica, buscamos usar frases cujo conteúdo textual fosse pouco expressivo, ou seja, onde o efeito \textit{linguístico} fosse mínimo. Inversamente, as frases foram lidas com a voz carregada de emoção, ou seja, com uma dramaticidade vocal que produzisse grandes variações visuais no desenho tipográfico. Optamos por trabalhar com bases de frases/áudio previamente criadas, já que explorar os meandros da confecção de uma fugiria ao cerne de questões que queremos explorar.

Assim, mesmo que criada sob motivações diversas às nossas, optamos pela base descrita e criada em \citet{pdpcosta2015}. Nela, e cumprindo o primeiro requisito, foram criadas 10 frases sem relação entre si e ``neutras, de modo que cada uma permita [pelos atores] a interpretação de diferentes emoções''. Essas frases foram então, e cumprindo nosso segundo requisito, lidas por 4 atores, cada um buscando representar as 6 emoções da tipologia ``Big Six'' \citep{ekman1970universal} (tristeza, medo, surpresa, repulsa, raiva e alegria).~\sidenote{A base inclui também leituras neutras, que usamos no segundo experimento.} Escolhemos quatro frases e, para minimizar variações entre leitores, fixamos em uma única atriz.

\begin{marginfigure}
  \includegraphics{imgs/exemplos-frases.png}
  \caption{Exemplo de cada frase usada nos cartões.}
...
Para a avaliação realizamos sessões de \textit{card sorting}. Eram individuais e divididas, cada uma, em quatro rodadas, uma para cada uma das frases escolhidas. Optamos por esse método por três principais motivos:

\begin{enumerate}

\item como discutido em \citet{goodman2006card}, trata-se de um método relativamente rápido e barato para se investigar constructos latentes em avaliações com participantes (em nosso caso, os constructos sendo as próprias classes de emoções);

\item sendo um teste presencial, nos permitiu observar os participantes e realizar breves entrevistas semi-estruturadas ao final de cada sessão, complementando os dados quantitativos coletados;

\item permite usar duas ferramentas de organização e interpretação de dados (\textit{matrizes de confusão} e \textit{edit-distances}) úteis porque, para além de uma mera taxa de ``acerto'' para as classificações, nos permitiriam quantificar a presença (ou não) de coerência na maneira como os participantes interpretaram cada cartão e emoção.


\end{enumerate}

Antes do início de cada sessão os participantes foram informados de que haveria nos cartões uma correspondência entre a forma tipográfica e certas características da voz de uma atriz que lera previamente os textos -- não informamos maiores detalhes. Para cada emoção havia um envelope rotulado e os participantes foram instruídos a depositar cada cartão na ``emoção'' correspondente a seu desenho tipográfico. 

Ao final de cada sessão realizamos breves entrevistas semi-estruturadas, buscando levantar possíveis estratégias usadas pelos participantes na organização dos cartões, além de investiga possíveis modelos mentais formulados para explicar o funcionamento do mapeamento fala-tipografia.

%Na Figura 4, uma fotografia tirada em uma das sessões.
