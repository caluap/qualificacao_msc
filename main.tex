...
Os resultados do segundo experimento, sintetizados na tabela~\ref{tab:type_perf}, são mais esclarecedores. Ao cruzar como interagem as três dimensões de emoção na voz, \textit{feature} prosódica representada e eixo tipográfico, é possível perceber que as preferências dos participantes não seguem um padrão único, variando de acordo com a configuração dessas dimensões. Assim, pode-se vislumbrar que uma representação bem sucedida de qualidades expressivas da prosódia teria que ponderar os atributos dessa mesma prosódia para só então determinar quais eixos tipográficos modular.
...
\begin{table}
    ...
    \begin{tabular}{lcccc}
        ...
        \midrule
        emoção & peso & larg. horiz. & inclinação & desloc. vert.  \\
        \midrule
...
Essa constatação, se verdadeira, carrega uma explicação possível para pelo menos parte do insucesso na abordagem testada no primeiro experimento: ao combinar de maneira injudiciosa as três \textit{features} prosódicas sempre associadas aos mesmos três eixos tipográficos, poderia-se estar indo contra certas expectativas dos participantes e, assim, induzindo-os ao erro na hora de classificar os cartões. 
...
Outra conclusão interessante é que tanto \textit{largura horizontal} quando \textit{inclinação}, pouco citados nas entrevistas do primeiro experimento, tem em geral uma performance mediana, com a preferência geral tendendo à inclinação apenas quando esta é associada a uma ``emoções'' bem peculiar: a voz neutra. De fato, como nota-se que na figura~\ref{slight_slant}, eleita a representação mais fiel da voz neutra, as modulações tipográficas aqui são praticamente imperceptíveis. Talvez, pode-se especular, foram eleitas pelos participantes justamente porque, em uma voz que busca ser inexpressiva, a melhora representação é aquela na qual não haja modulação tipográfica. Se for mesmo esse o caso, pode-se imaginar que uma abordagem que pondere os atributos prosódicos para só então determinar quais eixos tipográficos modular deve considerar também a possibilidade de, para determinadas vozes, \textit{não modular eixo algum}.
