...
Para analisar os resultados, decidimos pelo método de comparação de \textit{edit distances}. Esta é uma medida quantitativa na qual é obtida a soma de operações de transformação necessárias para se converter um arranjo de cartões em outro, ou seja, um índice da divergência entre os conjuntos -- útil para medirmos quão convergente cada participante foi em relação às ``Big Six.'' \citep{nawaz2012}
...
O segundo experimento foi planejado após percebermos, pela coleta e análise de dados do primeiro, que seria necessária uma investigação de foco mais estreito. Como nos dados que tínhamos então não conseguimos isolar o impacto de cada eixo tipográfico como representação de cada \textit{feature} prosódica para cada emoção, no segundo experimento pretendemos isolá-los, explorando, um-a-um, como esses parâmetros interagiam. 
...
Além dos três eixos tipográficos já testados no primeiro experimento, introduzimos um quarto, que chamamos de \textit{baseline shift}, ou deslocamento de linha de base. Nos inspiramos pela notação informal de melodia em canções brasileiras usada por \citet{tatit} e, como tal, uma possível notação intuitiva que, supomos, seria especialmente adequada para representação de \textit{f0}. Na figura~\ref{mods-exp-2} vemos as quatro modulações tipográficas.
...
A Figura~\ref{screenshot} mostra uma captura de tela do teste. Nela, dois exemplares da tipografia modulada eram exibidas, ambas contendo a mesma frase.

% Figure~\ref{fig:test_screenshot} shows a screenshot of the evaluation test. Two text boxes were shown to the participant. The text boxes contained two different versions of the same sentence, each one with letter shapes changed by one of the four typographic modulations evaluated (font weight, letter width, letter slant or baseline shift). The participant was able to hear the corresponding audio that generated the modulated sentences through the audio player control above the text boxes (Figure~\ref{fig:test_screenshot}). After hearing the audio (as many times as they wanted), participants were asked to choose which card they felt best represented it. 

% The total number of stimuli can be computed considering five emotions (surprise, happiness, neutral, sadness, anger), four typographic modulation axes and two sentences (``filha'', ``passarinho''), totalling 40 stimuli, resulting in 60 possible comparison pairs. However, after some pilot tests, we made each participant evaluate 30 different pairs of modulated sentences, as a good compromise between test coverage and feasibility. The order and exact pairings each participant were shown were randomized to control for their initial ineptitude and eventual tiredness (both the average time spent per evaluated pair and the number of times the audio was played had consistently high values for first five rounds, with a sharp drop and subsequent plateauing until round 20, slowly trailing off after that).
